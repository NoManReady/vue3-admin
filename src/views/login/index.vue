	...
	<div class="login-container">
		<div class="login-icon-group">
			<div class="login-icon-group-title">
				...
				<div class="login-icon-group-title-text font25">{{ themeConfigValue.globalSubTitle }}</div>
			</div>
			<img :src="loginIconTwo" alt="logo two" class="login-icon-group-icon" />
		</div>
		<div class="login-content">
			<div class="login-content-main">
				<h4 class="login-content-title ml15">{{ themeConfigValue.golbalTitle }}</h4>
				<Account />
			</div>
		</div>
	</div>
...
export default defineComponent({
	...
	setup() {
		...
		const themeConfigValue = computed(() => themeConfig.value)
		onMounted(() => {
			NextLoading.done()
		})
		return {
